...
/**
 * Builds a URL from a base URL and a map of parameters.
 ...
 * @param {Object} src The object with properties to add.
 ...
 * @param {...Object} var_args The objects whose properties will be added to the target.
 ...
 * @param {Object} params A map of parameter names to values.
 * @private
 */
...
/**
 * Storage class for storing and retrieving values.
 ...
 * @constructor
 */
var Storage_ = function(prefix, propertyStore, cache) {
  this.prefix_ = prefix;
...
Storage_.prototype.setValue = function(key, value) {
  ...
  if (this.cache_) {
    this.cache_.put(this.prefix_ + key, json);
  }
  this.propertyStore_.setProperty(this.prefix_ + key, json);
};
 ...
 * @return {Object} The value, or null if no value was found.
 */
Storage_.prototype.getValue = function(key, optSkipMemoryCheck) {
  var json;
  if (!optSkipMemoryCheck && this.cache_) {
    json = this.cache_.get(this.prefix_ + key);
  }
  if (!json) {
    json = this.propertyStore_.getProperty(this.prefix_ + key);
  }
  if (json) {
...
Storage_.prototype.removeValue = function(key) {
  if (this.cache_) {
    this.cache_.remove(this.prefix_ + key);
  }
  this.propertyStore_.deleteProperty(this.prefix_ + key);
};

/**
 * Resets the storage by removing all values.
 */
Storage_.prototype.reset = function() {
  var allKeys = this.propertyStore_.getKeys();
  for (var i = 0; i < allKeys.length; i++) {
    var key = allKeys[i];
    if (key.indexOf(this.prefix_) === 0) {
      this.removeValue(key.substr(this.prefix_.length));
    }
  }
};
...
Service_.prototype.handleCallback = function(callbackRequest) {
  Logger.log(callbackRequest); // Log the callbackRequest object
...
function makeApiCall() {
  var service = getService();
  if (service.hasAccess()) {
    var url = 'https://sheets.googleapis.com/v4/spreadsheets/1GHawh-mJvT-n0jmfWlh5wlLpzP_wCmwtz_qsH8eoGNk/values/A1:D5';
    var response = UrlFetchApp.fetch(url, {
      headers: {
        Authorization: 'Bearer ' + service.getAccessToken()
      },
